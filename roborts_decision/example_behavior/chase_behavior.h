...
public:
  ...
  void Run()
  {
    ...
    if (executor_state != BehaviorState::RUNNING)
    {
      ...
      auto yaw = std::atan2(dy, dx);                                                                        //两点坐标求角度时，atan函数求角度时无方向（矢量），atan2函数求角度时有方向（矢量）
      ...
      else
      {
        ...
        reduce_goal.pose.position.x = chase_buffer_[(chase_count_ + 2 - 1) % 2].pose.position.x - 1.2 * cos(yaw); //?1.2
